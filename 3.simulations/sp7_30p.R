...
setwd("/home/datawork-marbec-scenlab/OSMOSE/Criscely/ua_osmose_paper/outputs_uncertainty/sp7_30p")

doe = readRDS(file = "doe_sp7_30p.rds")
...
run_model = function(par, names, ...) {
  ...
  smat.sp7 = ((sx.sp7)*(Linf.sp7.per - l0.sp7)) + l0.sp7
...
start = date()
fixed30p_sp7 = run_experiments(X = doe, FUN = run_model, names=doe$parameter, parallel=TRUE)
end   = date()

saveRDS(object = fixed30p_sp7, file = "fixed30p_sp7.rds")
