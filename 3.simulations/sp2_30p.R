...
setwd("/home/datawork-marbec-scenlab/OSMOSE/Criscely/ua_osmose_paper/outputs_uncertainty/sp2_30p")

doe = readRDS(file = "doe_sp2_30p.rds")
...
run_model = function(par, names, ...) {
  ...
  smat.sp2 = ((sx.sp2)*(Linf.sp2.per - l0.sp2)) + l0.sp2
...
start = date()
fixed30p_sp2 = run_experiments(X = doe, FUN = run_model, names=doe$parameter, parallel=TRUE)
end   = date()

saveRDS(object = fixed30p_sp2, file = "fixed30p_sp2.rds")
